...
public class Coordinator {
    ...
    private static void runWithTiming(TaggedRunnable taggedRunnable) {
        ...
        try {
            taggedRunnable.run();
            if (isDebug) {
                System.out.println("Timing - " + Thread.currentThread().getName() + " " + taggedRunnable.tag + ": " + ((Debug.threadCpuTimeNanos() - j) / 1000000) + "ms (cpu) / " + ((System.nanoTime() - nanoTime) / 1000000) + "ms (real)");
                
            }
        } catch (RuntimeException e) {
           System.out.println( "Exception in " + taggedRunnable.tag);;
            if (isDebug) {
              System.out.println("Timing - " + Thread.currentThread().getName() + " " + taggedRunnable.tag + " (failed): " + ((Debug.threadCpuTimeNanos() - j) / 1000000) + "ms (cpu) / " + ((System.nanoTime() - nanoTime) / 1000000) + "ms (real)");
            }
        } catch (Throwable th) {
            Throwable th2 = th;
            int i = 1;
            Throwable th3 = th2;
            if (isDebug) {
                System.out.println("Timing - " + Thread.currentThread().getName() + " " + taggedRunnable.tag + (i != 0 ? " (failed): " : ": ") + ((Debug.threadCpuTimeNanos() - j) / 1000000) + "ms (cpu) / " + ((System.nanoTime() - nanoTime) / 1000000) + "ms (real)");;
            }
        }
    }
    ...
    static {
        ...
        ThreadPoolExecutor threadPoolExecutor = new ThreadPoolExecutor(8, 16, 1, TimeUnit.SECONDS, mPoolWorkQueue, new b(), new CoordinatorRejectHandler());
